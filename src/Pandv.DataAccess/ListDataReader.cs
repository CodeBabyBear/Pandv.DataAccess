...
namespace Pandv.DataAccess
{
    public class ListDataReader<T> : DbDataReader
    {
        ...
        protected List<Func<T, object>> _Getters = new List<Func<T, object>>();
        ...
        public ListDataReader(IEnumerable<T> data)
        {
            ...
            foreach (var property in properties)
            {
                ...
                var func = Expression.Lambda<Func<T, object>>(Expression.Convert(Expression.Property(v, property), typeof(object)), v).Compile();
                _Getters.Add(func);
            }
        }
